...
% A few macros copied verbatim from the paper's preamble (main.tex), so that snippets
% can be pasted straight from the manuscript into a response without redefining them.
...
\noindent
\textbf{Status.} Items whose response below begins ``Done'' or \done\ have been implemented in
the manuscript and verified against a clean build; the \done\ items are the ones
settled by the new introductory paragraphs on why \emph{Lean} was chosen and on
project logistics. \textbf{R2.T4} and \textbf{R2.T6} are
claimed and in progress. Everything else is still \todo.
...
\response{\done\ Noted in the Formal foundations section}

\item Explain how the millions of theorem statements were generated, and why that
generation process can be trusted.
\response{\done\ The Formal Foundations section was expanded to mention that the statements of machine-generated theorems were verified to follow a prescribed format using the \texttt{@{[}equational\_result{]}} attribute.}
...
\response{\done\ The introduction has been expanded with three new paragraphs that analyse
the collaboration and compare it with its predecessors: why \emph{Lean} was chosen, what
the GitHub/Zulip/blueprint paradigm did and did not achieve, and how the ETP's modularity
compares with both the Busy Beaver Challenge and earlier formalization projects. The
comparison is drawn along several concrete axes --- degree of centralization, the point at
which a blueprint becomes necessary, the coordination platforms used, and the division of
labour between informal proposers and formalizers --- rather than being confined to a
single remark. We have kept this in the introduction rather than giving it a separate
section, so that a reader meets it before the technical material.}
...
\response{\done\ The introduction now names the Liquid Tensor Experiment first among the
Lean formalization projects the ETP built on, ahead of PFR, and a new paragraph states
that the ETP's experience ``confirms the previous experiences with such structures'' in
the LTE and PFR. This adopts the framing the referee themselves proposed as the accurate
one.}
...
\response{\done\ The BB(5) discussion now records that the project was organized via a
Discord and a wiki, and that it ``was not initially developed as a formalization
project'', with the formal proof following afterwards. The contrasting ETP stack ---
GitHub repository, Zulip channel and Lean blueprint --- is described in the paragraph
that follows.}
...
\response{\done\ All four of the projects the referee names --- Flyspeck, the odd order
theorem formalization, the AFP and math-comp --- are now cited in the introduction and
contrasted with the ETP, which was able to decentralize further, and to do so before any
blueprint had been written. We also record the observation that the ETP's modularity, in
combination with a simple numerical completion criterion, is what permitted this, and
suggest that future crowdsourced projects design modularity into their goals deliberately.
We have not attempted the referee's further comparison with large-scale software and proof
assistant development, which we felt would take the introduction some distance from the
mathematics; we would be glad to add it if the referee considers it important.}
...
\response{\done\ We agree that this was the wrong place for the observation, and it has been
promoted to a paragraph of its own in the introduction, where it is now stated as one of
the project's findings: that the success of the ETP was contingent on an
\emph{intermediate} level of automation, enough to clear the millions of routine
implications but not so rapid as to crowd out the slower and more fruitful search for
conceptually novel arguments. The fuller discussion in \S14.1 is cross-referenced from
there and left in place.}
...
\response{\done\ A new paragraph in the introduction answers this directly: \emph{Lean} was
chosen for the existing crowdsourced formalization projects in it and the infrastructure
built around them, rather than for any technical property of the system; and, since the
project needed relatively little advanced mathematics, comparable results could likely
have been obtained with another proof assistant.}
...
\response{\done\ The introduction now grants that \emph{Isabelle} would have been a
particularly plausible alternative for this project, because of its native support for SMT
solvers via \emph{Sledgehammer}, and draws the contrast the referee intends: for the ETP,
custom conversions from ATP-generated proofs into Lean code had to be written instead, as
described in \S7.}
...
this project.
\response{\done\ Conceded explicitly in the introduction: the project could likely have been
carried out with ``other standard formal proof assistants that have not developed as
extensive a library of mathematics as Lean's \emph{Mathlib}''.}
...
\response{\done\ The Formal foundations section was corrected to mention that the axiom check was performed by the \texttt{@{[}equational\_result{]}} attribute and to address axiom set.}
...
\response{Timing and line counts for the reduced set were included in the Formal foundations section \todo\ Transitivity and duality}
...
interactive theorem prover Lean'', to match the terminology of \S4.7.
\response{\done}
...
\response{\done (the discrepancy was indeed for this reason)}
...
\response{\done (This was a typo rather than a second tool.)}
